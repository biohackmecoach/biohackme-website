...
function getMetadataForPath(requestPath) {
    // Normalize path
...
function generateCrawlerHtml(metadata, requestPath) {
...
async function getBaseHtml() {
    ...
    try {
        ...
        const response = await (0, node_fetch_1.default)("https://www.biohackme.com.au/__internal/base.html", {
            headers: {
...
function injectOgTags(html, metadata, requestPath) {
    const canonicalUrl = `https://www.biohackme.com.au${requestPath}`;
    ...
    // Replace canonical
    modifiedHtml = modifiedHtml.replace(/<link rel="canonical" href="[^"]*"/, `<link rel="canonical" href="${canonicalUrl}"`);
    // Replace OG tags
    modifiedHtml = modifiedHtml.replace(/<meta property="og:url" content="[^"]*"/, `<meta property="og:url" content="${canonicalUrl}"`);
    ...
    modifiedHtml = modifiedHtml.replace(/<meta name="twitter:title" content="[^"]*"/, `<meta name="twitter:title" content="${metadata.title}"`);
    modifiedHtml = modifiedHtml.replace(/<meta name="twitter:description" content="[^"]*"/, `<meta name="twitter:description" content="${metadata.description}"`);
    modifiedHtml = modifiedHtml.replace(/<meta name="twitter:image" content="[^"]*"/, `<meta name="twitter:image" content="${metadata.image}"`);
    return modifiedHtml;
}
exports.ogMiddleware = functions.https.onRequest(async (req, res) => {
